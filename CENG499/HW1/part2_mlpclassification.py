...
def forward_pass(w1, b1, w2, b2, input_data):
    ...
    layer1 = torch.sigmoid(torch.matmul(w1, input_data.T) + b1)
    layer2 = torch.matmul(w2, layer1) + b2
    return torch.softmax(layer2.T, dim=1)
...
w1 = torch.normal(mean=0, std=1, size=(16, 3), requires_grad=True)
...
#b1 = torch.empty(16).normal_(mean=0, std=1)
b1 = torch.normal(mean=0, std=1, size=(16,1), requires_grad=True)
...
w2 = torch.normal(mean=0, std=1, size=(3, 16), requires_grad=True)
...
b2=torch.normal(mean=0, std=1, size=(3, 1), requires_grad=True)
#b2 = torch.empty(3).normal_(mean=0, std=1)
...
for iteration in range(1, ITERATION+1):
    ...
    with torch.no_grad():
        train_prediction_outputs = torch.argmax(train_predictions, dim=1)
        train_label_outputs = torch.argmax(train_label, dim=1)

        # Here you are expected to calculate the accuracy score on the training dataset by using the training labels.
        train_accuracy = 100 * torch.sum((train_prediction_outputs == train_label_outputs).int()) / train_dataset.shape[0]
        ...
        validation_prediction_outputs = torch.argmax(validation_predictions, dim=1)
        validation_label_outputs = torch.argmax(validation_label, dim=1)
        # Similarly, here, you are expected to calculate the accuracy score on the validation dataset
        validation_accuracy = 100 * torch.sum((validation_prediction_outputs == validation_label_outputs).int()) / validation_dataset.shape[0]
...
with torch.no_grad():
    ...
    test_prediction_outputs = torch.argmax(test_predictions, dim=1)
    test_label_outputs = torch.argmax(test_label, dim=1)
    # Here you are expected to calculate the network accuracy score on the test dataset...
    test_accuracy = 100 * torch.sum((test_prediction_outputs == test_label_outputs).int()) / test_dataset.shape[0]
