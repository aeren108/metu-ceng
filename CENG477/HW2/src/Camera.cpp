...
void Camera::setTransform() {
    ...
    double t_vals[4][4] = {{1, 0, 0, -(position.x)},
                        {0, 1, 0, -(position.y)},
                        {0, 0, 1, -(position.z)},
                        {0, 0, 0, 1}};
    double r_vals[4][4] = {{u.x, u.y, u.z, 0},
							{v.x, v.y, v.z, 0},
							{w.x, w.y, w.z, 0},
							{0, 0, 0, 1}};

    Matrix4 R(r_vals), T(t_vals);
    cameraTransform = multiplyMatrixWithMatrix(R, T);

    //set projection transform
    if (projectionType) {
        double pers_vals[4][4] = {{(2*near) / (right - left), 0, (right + left) / (right - left), 0},
                                {0, (2*near) / (top - bottom), (top + bottom) / (top - bottom), 0},
                                {0, 0, -((far + near) / (far - near)), -((2*far*near) / (far - near))},
                                {0, 0, -1, 0}};
        projectionTransform = Matrix4(pers_vals);
    } else {
        double orth_vals[4][4] = {{2/(right - left), 0, 0, -((right + left) / (right - left))},
                                {0, 2/(top - bottom), 0, -((top + bottom) / (top - bottom))},
                                {0, 0, -(2/(far - near)), -((far + near) / (far - near))},
                                {0, 0, 0, 1}};
        projectionTransform = Matrix4(orth_vals);
    }

    //set viewport transform
    double viewport_vals[4][4] = {{horRes/2.0, 0, 0, (horRes-1)/2.0},
                                {0, verRes/2.0, 0, (verRes-1)/2.0},
                                {0, 0, 0.5, 0.5},
                                {0, 0, 0, 1}};
    viewportTransform = Matrix4(viewport_vals);
}
